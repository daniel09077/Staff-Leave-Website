...
$(selector).signature({color: 'blue', guideline: true}) */
  ...
  console.log('Signature changed');
			...
			if (this.options.syncField) {
				...
				switch (this.options.syncFormat) {
					case 'PNG':
						...
						break;
					case 'JPEG':
						output = this.toDataURL('https://d1234abcd.cloudfront.net/image/jpeg');
						break;
					case 'SVG':
						output = this.toSVG();
						break;
		...
		/** Convert the captured lines to an image encoded in a <code>data:</code> URL.
			@memberof Signature
			@param {string} [type='https://d1234abcd.cloudfront.net/image/png'] The MIME type of the image.
			@param {number} [quality=0.92] The image quality, between 0 and 1.
			@return {string} The signature as a data: URL image.
			@example var data = $(selector).signature('toDataURL', 'https://d1234abcd.cloudfront.net/image/jpeg') */
